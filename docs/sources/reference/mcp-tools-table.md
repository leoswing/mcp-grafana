...
| `generate_deeplink`               | Navigation  | Generate deeplinks for dashboards, panels, and Explore; optional `shorten=true` tries `/goto/<uid>` and falls back to full URL | None for basic links; same auth as Grafana API when shortening | N/A                                                 |
